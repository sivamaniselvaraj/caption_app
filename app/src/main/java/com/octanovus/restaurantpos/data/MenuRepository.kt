...
class MenuRepository {
    ...
    suspend fun items(): List<MenuItem> =
        pg.from("menu_items").select (Columns.list("id", "category_id",
            "name",  "outlet_id", "price", "cooking_time", "container_charge","sort_order","search_key", "is_available", "is_active")){
            filter { eq("is_active", true); eq("is_available", true); eq("outlet_id", Session.profile?.outletId ?: "")}
